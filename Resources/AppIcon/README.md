...
| `Rumen-icon.svg` | Flat master of the same design, Apple's squircle tile baked in. |
| `Rumen-icon-small.svg` | Hand-tuned art for 16px and 32px: no grid, a heavier extent, a darker flap. |
| `Rumen.icns` | Legacy icns built from the two flat masters by `scripts/render-app-icon.sh`. For DMG art, docs, and non-Xcode packaging only. |

## Regenerating

```sh
node design/icon/export-app-icon.mjs design/icon/concepts.mjs peel Resources/AppIcon
scripts/render-app-icon.sh        # flat masters -> Rumen.icns
