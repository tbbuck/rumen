...
final class ExtractabilityTests: XCTestCase {
    ...
    func testHappyPathPBFOffsetPaging() {
        ...
        XCTAssertEqual(a.pageSize, 2000)
    ...
    func testTransportChoice() {
        ...
        XCTAssertEqual(json.reason, "JSON, offset paging at 2,000 records per request.")
    ...
    func testStrategyLadder() {
        let range = Extractability.assess(layer: layer(10, service: 1, paging: false, statistics: true), service: service(1, "A", .mapServer))
        XCTAssertEqual(range.strategy, .oidRange)
        XCTAssertEqual(range.reason, "PBF, OID range chunking at 2,000 records per request.")
        let list = Extractability.assess(layer: layer(10, service: 1, paging: nil, statistics: nil), service: service(1, "A", .mapServer))
        XCTAssertEqual(list.strategy, .oidList)
        ...
        let fallbackPage = Extractability.assess(layer: layer(10, service: 1, maxRecordCount: nil), service: service(1, "A", .mapServer))
    ...
    func testTwinIsPreferredWhenBetter() {
        ...
        XCTAssertEqual(a.transport, .pbf)
        XCTAssertEqual(a.strategy, .offset)
        XCTAssertEqual(a.reason, "PBF through the FeatureServer twin, offset paging at 2,000 records per request.")
    }
    ...
    func testFixtureLayers() throws {
        ...
        XCTAssertEqual(a.verdict, true)
        XCTAssertEqual(a.reason, "PBF, offset paging at 1,000 records per request.")
        let b = Extractability.assess(layer: record(trailheads, id: 2), service: service(1, "Trailheads", .featureServer))
        XCTAssertEqual(b.reason, "PBF, offset paging at 2,000 records per request.")
    }
}
