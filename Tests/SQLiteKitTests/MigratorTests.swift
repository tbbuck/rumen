...
final class MigratorTests: XCTestCase {
    ...
    private func tables(_ db: SQLite) throws -> [String] {
    ...
    private func recorded(_ db: SQLite) throws -> [Int] {
        try db.run("SELECT version FROM schema_migrations ORDER BY version;").rows.compactMap { $0.first?.intValue }
    }
    ...
    func testRejectsDuplicateVersions() {
        XCTAssertThrowsError(try Migrator.validated([Migration(version: 1, name: "a", sql: ""), Migration(version: 1, name: "b", sql: "")])) { error in
    ...
    func testAppliesInOrderAndRecords() throws {
        ...
        let names = try db.run("SELECT name FROM schema_migrations ORDER BY version;").rows.compactMap { $0.first?.stringValue }
        XCTAssertEqual(names, ["a", "b"])
        let appliedAt = try db.run("SELECT applied_at FROM schema_migrations WHERE version = 1;").rows[0][0].dateFromMicros
        XCTAssertEqual(appliedAt!.timeIntervalSinceNow, 0, accuracy: 5)
    }
    ...
    func testReapplyIsANoop() throws {
        ...
        XCTAssertEqual(try Migrator.apply(migrations, to: db), MigrationReport(applied: [], currentVersion: 1))
        XCTAssertEqual(try recorded(db), [1])
    }

    func testMultiStatementMigrationRunsWhole() throws {
        let db = try SQLite(path: ":memory:")
        try Migrator.apply([Migration(version: 1, name: "two", sql: """
    ...
    func testFailedMigrationRollsBackAndIsUnrecorded() throws {
        ...
        let migrations = [
            Migration(version: 1, name: "a", sql: "CREATE TABLE a (x INTEGER);"),
            Migration(version: 2, name: "broken", sql: "CREATE TABLE b (y INTEGER); SELECT * FROM does_not_exist;"),
        ]
    ...
    func testUnknownAppliedVersionIsRefused() throws {
        let db = try SQLite(path: ":memory:")
        try Migrator.apply([Migration(version: 1, name: "a", sql: "CREATE TABLE a (x INTEGER);"),
                            Migration(version: 2, name: "b", sql: "CREATE TABLE b (x INTEGER);")], to: db)
        XCTAssertThrowsError(try Migrator.apply([Migration(version: 1, name: "a", sql: "CREATE TABLE a (x INTEGER);")], to: db)) { error in
    ...
    func testOutOfOrderPendingIsRefused() throws {
        ...
        XCTAssertThrowsError(try Migrator.apply([Migration(version: 1, name: "a", sql: "CREATE TABLE a (x INTEGER);"),
                                                 Migration(version: 2, name: "b", sql: "CREATE TABLE b (x INTEGER);")], to: db)) { error in
